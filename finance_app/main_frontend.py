...
def main_frontend(**opts):
    ...
    # Top summary
    st.markdown("""---""")
    agg_upto_df, agg_on_df = get_client_positions_from_top_summary(clients=advisor_clients, positions=positions)
    st.markdown("""---""")
    ...
    with col3:
        pie = get_pie_chart(agg_on_df, label_col=st.session_state["aggregation_level"], values_col="Market Value")
